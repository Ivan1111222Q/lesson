...
# Обновлять values.yaml (только image.repository и image.tag)
update_values: true
```
...
| `helm_chart_path` | ✅ | Путь к Helm chart (относительно скрипта) |
| `helm_release` | ✅ | Имя Helm release |
| `helm_namespace` | ✅ | Kubernetes namespace |
| `auto_version` | ❌ | Автоинкремент версии (по умолчанию `false`) |
| `update_values` | ❌ | Обновлять values.yaml (по умолчанию `false`) |
